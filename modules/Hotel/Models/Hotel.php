...
use App\Models\Location as BravoLocation;
...
class Hotel extends Bookable
{
    ...
    public function getRoomsAvailability($filters = [])
    {
        ...
        //$check_date = date("Y-m-d", strtotime("+24 Hours"));
        $check_date = $filters['start_date'];
        ...
        foreach ($rooms as $room) {
            if ($room->isAvailableAt($filters)) {
                ...
                // ];
                $media = MediaFile::find($room->image_id);
                $pre_url = 'https://cdn.mykomodo.kabtour.com/uploads/';
                if($media){
                    $room->banner = [
                        "original"=> $media->file_path != null ? $pre_url.$media->file_path : null,
                ...
                $tempgal = explode(',',$room->gallery);
                $arrGall = [];
                for($i=0;$i<sizeof($tempgal);$i++){
                    $gal = MediaFile::find($tempgal[$i]);
                    if($gal){
                        $arrGall[] = [
                            "original"=> $gal->file_path != null ? $pre_url.$gal->file_path : null,
                ...
                $room->gallery = $arrGall;
                $room->price_weekday = $room->price;
                $room->discount_weekday = $room->discount;
                $room->price = $room->price_weekday;
                $room->discount = $room->discount_weekday;
                $room->price_final = $room->price_weekday - $room->discount_weekday;
                if ($room->price_weekend && $room->price_weekend > 0) {
                    if (date("D", strtotime($check_date)) === "Sat" || date("D", strtotime($check_date)) === "Sun") {
                        $room->price = $room->price_weekend;
                        $room->discount = $room->discount_weekend;
                        $room->price_final = $room->price_weekend - $room->discount_weekend;
                    }
                    if (array_key_exists($check_date, $get_holiday)) {
                        $room->price = $room->weekend;
                        $room->discount = $room->discount_weekend;
                        $room->price_final = $room->price_weekend - $room->discount_weekend;
                    }
                }
                $getCalendars = BravoPriceCalendar::where(['create_user'=>$room->create_user,'object_model'=>'hotel','price_date'=>$check_date,'status'=>1])->first();
                if ($getCalendars) {
                    if ($room->price_holiday && $room->price_holiday > 0) {
                        $room->price = $room->price_holiday;
                        $room->discount = $room->discount_holiday;
                        $room->price_final = $room->price_holiday - $room->discount_holiday;
                    }
                }
                $room->tmp_price = $room->price_final*$room->tmp_nights;
                $room->price_final = (string)$room->price_final;
                $res[] = [
                    'id'              => $room->id,
                    'title'           => $translation->title,
                    // 'price'           => $room->tmp_price ?? 0,
                    ...
                    'terms'           => $terms,
                    'term_features'   => $term_features,
                    //'price_normal'    => $room->price_normal,
                    //'price_weekend'   => $room->price_weekend,
                    //'price_holiday'   => $room->price_holiday,
                    'discount'        => $room->discount,
                    //'discount_weekend'=> $room->discount_weekend,
                    //'discount_holiday'=> $room->discount_holiday,
                    'price'           => $room->price,
    ...
    public static function search(Request $request)
    {
        ...
        if(!empty($create_user = $request->query('create_user'))){
            $model_hotel->where('bravo_hotels.create_user',$create_user);
        }

        if($request->has('keyword')){
            $model_hotel->where('bravo_hotels.title','LIKE','%'.$request->keyword.'%');
        }
